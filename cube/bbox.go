...
func Box(x0, y0, z0, x1, y1, z1 float32) BBox {
	if x0 > x1 {
		x0, x1 = x1, x0
	}
	if y0 > y1 {
	...
	return BBox{min: mgl32.Vec3{x0, y0, z0}, max: mgl32.Vec3{x1, y1, z1}}
}

// Grow grows the bounding box in all directions by x and returns the new bounding box.
func (box BBox) Grow(x float32) BBox {
...
func (box BBox) Extend(vec mgl32.Vec3) BBox {
	if vec[0] < 0 {
		box.min[0] += vec[0]
	} else if vec[0] > 0 {
		box.max[0] += vec[0]
...
func (box BBox) Stretch(a Axis, x float32) BBox {
	switch a {
	case Y:
		box.min[1] -= x
		box.max[1] += x
...
func (box BBox) Translate(vec mgl32.Vec3) BBox {
...
func (box BBox) TranslateTowards(f Face, x float32) BBox {
	switch f {
	case FaceDown:
		return box.Translate(mgl32.Vec3{0, -x, 0})
	case FaceUp:
		return box.Translate(mgl32.Vec3{0, x, 0})
	case FaceNorth:
		return box.Translate(mgl32.Vec3{0, 0, -x})
	case FaceSouth:
		return box.Translate(mgl32.Vec3{0, 0, x})
	case FaceWest:
		return box.Translate(mgl32.Vec3{-x, 0, 0})
	case FaceEast:
		return box.Translate(mgl32.Vec3{x, 0, 0})
	}
	return box
...
// the case.
func (box BBox) intersectsWith(other BBox, epsilon float32) bool {
...
func (box BBox) Vec3Within(vec mgl32.Vec3) bool {
...
func (box BBox) Vec3WithinYZ(vec mgl32.Vec3) bool {
	if vec[2] < box.min[2] || vec[2] > box.max[2] {
		return false
	}
	return vec[1] >= box.min[1] && vec[1] <= box.max[1]
...
func (box BBox) Vec3WithinXZ(vec mgl32.Vec3) bool {
	if vec[0] < box.min[0] || vec[0] > box.max[0] {
		return false
	}
...
func (box BBox) Vec3WithinXY(vec mgl32.Vec3) bool {
	if vec[0] < box.min[0] || vec[0] > box.max[0] {
		return false
	}
	return vec[1] >= box.min[1] && vec[1] <= box.max[1]
...
// is smaller than 0.
func (box BBox) XOffset(nearby BBox, deltaX float32) float32 {
...
// is smaller than 0.
func (box BBox) YOffset(nearby BBox, deltaY float32) float32 {
...
// is smaller than 0.
func (box BBox) ZOffset(nearby BBox, deltaZ float32) float32 {
